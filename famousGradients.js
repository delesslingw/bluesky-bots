...
async function prepareResources() {
    const artData = await getArt(); // Get art data
    console.log("Art Data:", artData);

    // Download the image locally before using it in the sketch
    const fileName = "bin/" + `${artData.name}___${artData.artistName}___${artData.year}`;
...
async function createCanvasArt() {
    ...
    const outputFileName = `bin/output_${artData.name}.png`;
